...
def train(num_gpus, rank, group_name, 
          exp_path, log, optimization, device=None):
    ...
    opt_betas = (optimization["betas"][0], optimization["betas"][1])
    opt_eps = optimization["eps"]
    opt_weight_decay = optimization["weight_decay"]
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate, betas=opt_betas, eps=opt_eps, weight_decay=opt_weight_decay)
    ...
    while n_iter < optimization["n_iters"] + 1:
        ...
        for x, y in trainloader:             
            noisy_spec = x.to(device)  # Shape: (batch_size, freq_bins, time_steps)
            clean_spec = y.to(device)
